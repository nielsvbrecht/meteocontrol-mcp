...
describe('Asset Information Tool', () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    new MeteoControlServer();
  });

  it('should register the get_asset_info tool', async () => {
    ...
    expect(result.tools).toContainEqual(expect.objectContaining({ name: 'get_asset_info' }));
  });

  it('should fetch asset info for a system', async () => {
    const mockData = { result: 'ok', assets: [] };
    ...
    expect(mockGet).toHaveBeenCalledWith('/systems/sys1', {});
    expect(result.content[0]!.text).toBe(JSON.stringify(mockData, null, 2));
  });

  it('should handle errors when fetching asset info', async () => {
    ...
    expect(result.content[0]!.text).toContain('Error fetching asset info: API Error');
  });
});
